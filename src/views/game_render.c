...
/*   Updated: 2025/01/29 06:54:53 by mfidimal         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	frame_render(t_data *data, int x, int y)
{
	...
	if (element == '0')
		mlx_put_image_to_window(data->mlx, data->mlx_win, data->road, x
			* FRAME_SIZE, y * FRAME_SIZE);
	if (element == 'C')
		mlx_put_image_to_window(data->mlx, data->mlx_win, data->collectible_obj,
			x * FRAME_SIZE, y * FRAME_SIZE);
	if (element == 'P')
		mlx_put_image_to_window(data->mlx, data->mlx_win, data->player, x
			* FRAME_SIZE, y * FRAME_SIZE);
	if (element == 'E')
